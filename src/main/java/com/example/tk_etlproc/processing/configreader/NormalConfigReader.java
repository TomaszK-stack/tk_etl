    ...
    private BaseStep getStep(String stepName, List<String> stepMetaList) throws StepNotFoundException {
        switch (stepName.toLowerCase()) {
            case "nullif":
                ...
                NullifStepMeta nullifStepMeta = NullifStepMeta.builder().columnName(stepMetaList.get(0)).columnNameValueExpression(stepMetaList.get(1)).valueLogicExpression(stepMetaList.get(2)).build();
                nullifStep.setNullifStepMeta(nullifStepMeta);
                return nullifStep;
            case "joincolumn":
                JoinColumnsStep joinColumnsStep = new JoinColumnsStep();
